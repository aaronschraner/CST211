...
mkclass()
{
	...
	#includes
	echo "List the files / libraries you want to include in the declaration. (<libname> for external libs and \"filename\" for local files)"
	includefiles "$classdecfile"
	...
	then
		...
		read tempnames
		tempargs="template < typename $(echo $tempnames | sed 's/ /, typename /g') >"
...
#openheader >> testFile.cpp
#ioheader testFile.cpp Arg1 Arg2 Arg3
#closeheader >> testFile.cpp
